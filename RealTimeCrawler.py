...
def import_to_db(engine, data_model):
    ...
    for file_name in onlyfiles:
        ...
        #    continue
def refreshDB(session,data_model):
    html_src_dir = join(target_path,"ShoesDescriptor","html")
    dirs = [item for item in os.listdir(html_src_dir) if os.path.isdir(os.path.join(html_src_dir, item))] 
    for dir in dirs:
        dealer = dir
        files = [item for item in os.listdir(os.path.join(html_src_dir,dir)) if os.path.isfile(os.path.join(html_src_dir,dir, item))] 
        for file in files:
            html_file = join(html_src_dir,dir,file)
            with open(html_file,'r',encoding = 'utf-8') as fr:
                    html_source = fr.read()
                    tree = html.fromstring(html_source)
                    dic = {}
                    ...
                    for m in metas:
                        key = m.xpath('.//b/text()')[0]
                        if key[0] == "货":
                            dic["ID"] = m.xpath('.//strong/text()')[0]
                            #results = session.query(data_model.ID, data_model.ID)
                            results = session.execute("select ID from ShoesDescriptor where ID=\"{}\"".format(dic["ID"]))
                            if len(results) > 0:
                                continue
                        elif key[0] == "尺":
                            dic["Sizes"] = "|".join(m.xpath('.//ul/li/a/text()'))
